...
trials = 1
pressure = 760. * 1
file_name = 'charge_density/hole2/1atm.csv'
#directory = '/afs/cern.ch/user/b/bmcconne/private/garfieldpp/GEM/triple_gem/nd-gar/gap/tgap1/charge_density/hole2'
...
sensor.SetArea(-2, -1, -1, 2,  1, 0.005)                #0.204 for gem 3
...
for i in range(trials):
  ...
  for electron in aval.GetElectrons():
    ...
    if status_code == -1:
      total += 1
      if(np.sqrt(fx**2+fy**2)<=0.0035):
        countA +=1
      elif(np.sqrt(fx**2+fy**2)<=0.0175):
        countB += 1
      else:
        outside +=1 
    
  if(total!=0): 
    percentages.append((countA/total, countB/total, outside/total , total))  
  else:
    percentages.append((np.nan,np.nan,np.nan,np.nan)) 

tuples_to_csv(percentages,['A', 'B', 'C', 'Total'], file_name)

'''
cD = ROOT.TCanvas('cD', '', 600, 600)
meshView = ROOT.Garfield.ViewFEMesh()
meshView.SetArea(-0.2, -0.1, 0.2, 0.3)
...
cD = ROOT.TCanvas('cD', '', 600, 600)
meshView = ROOT.Garfield.ViewFEMesh()
meshView.SetArea(-0.1, -0.2, 0.1, 0.2)
meshView.SetCanvas(cD)
meshView.SetComponent(fm)
# x-z projection.
meshView.SetPlane(0, -1, 0, 0, 0, 0)
