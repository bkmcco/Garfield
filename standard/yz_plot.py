...
# === Read y and z coordinates from the CSV ===
y_vals = []
z_vals = []
...
with open(csv_file, newline='') as csvfile:
    ...
    for row in reader:
        y_vals.append(float(row["y"]))  # Use "y" instead of "x"
...
plt.scatter(y_vals, z_vals, color='blue', s=10, label='Electrons')

# Plot GEM layers
for name, z_pos in gem_layer_positions.items():
    plt.axhline(z_pos, color='gray', linestyle='--', alpha=0.7)
    plt.text(min(y_vals), z_pos + 0.002, name, verticalalignment='bottom', color='black')

# Labels and title
plt.xlabel("y [cm]")
plt.ylabel("z [cm]")
plt.title("y-z GEM 3 (2 atm)")
plt.grid(True)
plt.tight_layout()
